...
public class Matrix3DReportService
{
        ...
        double mse)
    {
        ...
        sb.AppendLine("    :root { --ink:#172033; --muted:#64748b; --line:#e2e8f0; --accent:#4f46e5; }");
        sb.AppendLine("    body { padding:34px clamp(20px,4vw,64px) 64px; background:radial-gradient(circle at 78% 0%,#eef2ff 0,transparent 30%),#f4f7fc; color:var(--ink); }");
        sb.AppendLine("    .container { max-width:1500px; margin:0 auto; }");
        sb.AppendLine("    .header { position:relative; overflow:hidden; padding:32px 34px 28px; border:0; border-radius:20px; background:linear-gradient(135deg,#172033 0%,#263454 62%,#3730a3 145%); box-shadow:0 18px 45px rgba(15,23,42,.15); }");
        sb.AppendLine("    .header::after { content:''; position:absolute; width:320px; height:320px; border-radius:50%; right:-130px; top:-165px; background:rgba(129,140,248,.18); }");
        sb.AppendLine("    .eyebrow { position:relative; z-index:1; margin:0 0 12px; color:#a5b4fc; font-size:11px; font-weight:800; text-transform:uppercase; letter-spacing:.14em; }");
        sb.AppendLine("    h1 { position:relative; z-index:1; color:#fff; font-size:clamp(24px,3vw,34px); letter-spacing:-.03em; }");
        sb.AppendLine("    .header p { position:relative; z-index:1; max-width:980px; color:#cbd5e1; }");
        sb.AppendLine("    .header code { background:rgba(255,255,255,.09); border-color:rgba(255,255,255,.12); color:#fff; }");
        sb.AppendLine("    .meta-badges { position:relative; z-index:1; display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:10px; margin-top:22px; }");
        sb.AppendLine("    .badge { padding:12px 14px; border-radius:12px; background:rgba(255,255,255,.075); border-color:rgba(255,255,255,.10); color:#aab6ca; font-size:11px; }");
        sb.AppendLine("    .badge strong { display:block; margin-top:3px; color:#fff; font-size:14px; }");
        sb.AppendLine("    .plot-card,.info-card { border-color:rgba(226,232,240,.9); border-radius:16px; box-shadow:0 10px 30px rgba(15,23,42,.055); }");
        sb.AppendLine("    .plot-card { padding:22px; }");
        sb.AppendLine("    .info-card { padding:24px 28px; }");
        sb.AppendLine("    .info-card h3 { font-size:18px; letter-spacing:-.01em; }");
        sb.AppendLine("    .btn { padding:10px 16px; border-radius:10px; background:#4f46e5; border-color:#4f46e5; font-weight:650; box-shadow:0 6px 16px rgba(79,70,229,.18); }");
        sb.AppendLine("    .btn:hover { background:#4338ca; }");
        sb.AppendLine("    .btn-secondary { color:#334155; background:#fff; border-color:#e2e8f0; box-shadow:none; }");
        sb.AppendLine("    @media(max-width:900px){body{padding:18px}.meta-badges{grid-template-columns:1fr 1fr}.info-grid{grid-template-columns:1fr}#plot3d{height:560px}}");
        sb.AppendLine("  </style>");
        sb.AppendLine("</head>");
        sb.AppendLine("<body>");
        sb.AppendLine("  <div class=\"container\">");
        sb.AppendLine("    <div class=\"header\">");
        sb.AppendLine("      <div class=\"eyebrow\">Algorithm Lab · Пространственный эксперимент</div>");
        ...
        sb.AppendLine("        <button class=\"btn\" onclick=\"toggleTheory()\">Переключить теоретическую поверхность</button>");
        sb.AppendLine("        <button class=\"btn btn-secondary\" onclick=\"resetCamera()\">Сбросить камеру</button>");
        sb.AppendLine("      </div>");
