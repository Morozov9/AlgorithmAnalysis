...
/// <summary>
/// Сервис для автоматической генерации полного отчёта по лабораторной работе №1.
...
public class ReportService
{
    ...
    /// <summary>
    /// Генерирует полный отчёт для всех алгоритмов.
    /// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        for (int i = 0; i < total; i++)
        {
            ...
            // Подбираем сбалансированные размеры для быстрого и наглядного отчёта (15-25 точек)
        ...
        // Строим сводный график всех сортировок
        ...
        progress?.Invoke("Формирование HTML и Markdown файлов отчёта...", 0.95);
    ...
    /// <summary>
    /// Размеры данных для отчёта (обеспечивают высокое качество кривых при быстром расчёте).
    /// </summary>
    ...
    private static void SaveAlgorithmChart(AbstractAlgorithm algo, BenchmarkResult bench, string filePath)
    {
        ...
        theo.LegendText = $"Теоретическая кривая ({bench.ComplexityLabel}, c={bench.FittedCoefficient:E2})";
        ...
        string combinedSortChartPath)
    {
        ...
        sb.AppendLine("  <title>Отчёт по лабораторной работе №1 — Анализ временной сложности алгоритмов</title>");
        sb.AppendLine("  <style>");
        sb.AppendLine("    body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; line-height: 1.6; color: #24292e; max-width: 1100px; margin: 0 auto; padding: 25px; background: #fdfdfd; }");
        ...
        sb.AppendLine("  <button class=\"print-btn\" onclick=\"window.print()\">🖨 Печать в PDF / Принтер</button>");
        ...
        sb.AppendLine("    <p><b>Цель работы:</b> Практическое исследование зависимости времени работы алгоритмов от объема входных данных, сопоставление экспериментальных кривых с теоретическими классами сложности Big-O, аппроксимация методом наименьших квадратов.</p>");
        sb.AppendLine("  </div>");
        ...
        sb.AppendLine("    <p><b>Анализ результатов:</b> Из графика наглядно видно принципиальное преимущество алгоритмов класса <code>O(n log n)</code> (Quick Sort, Timsort, Merge Sort) над квадратичными алгоритмами <code>O(n²)</code> (Bubble Sort, Insertion Sort). При росте n до 2000 элементов время работы квадратичных сортировок растет круто по параболе, в то время как быстрая сортировка и Timsort выполняются практически мгновенно.</p>");
        sb.AppendLine("  </div>");

        // Результаты по всем алгоритмам
        ...
        foreach (var (algo, bench, chartPath) in results)
        {
            ...
            sb.AppendLine($"    <p><b>Группа:</b> {algo.Group} | <b>Коэффициент аппроксимации:</b> <code>c = {bench.FittedCoefficient:E3}</code> | <b>Формула теории:</b> <code>T = {bench.FittedCoefficient:E2} · f(n)</code></p>");
            sb.AppendLine($"    <img class=\"chart-img\" src=\"{chartPath}\" alt=\"{algo.Name}\">");

            sb.AppendLine("    <table>");
            sb.AppendLine("      <thead><tr><th>n</th><th>T эксп. (мс)</th><th>T теор. (мс)</th><th>Разброс по 5 запускам (мс)</th></tr></thead>");
            sb.AppendLine("      <tbody>");
        ...
        // Выводы
        sb.AppendLine("  <h2>Выводы по работе</h2>");
        sb.AppendLine("  <div class=\"algo-card\">");
        sb.AppendLine("    <ol>");
        sb.AppendLine("      <li><b>Соответствие теории и практики:</b> Экспериментальные замеры времени подтверждают теоретические классы сложности. Аппроксимация методом наименьших квадратов позволила сопоставить формулу <code>T = c · f(n)</code> с реальным временем исполнения.</li>");
        sb.AppendLine("      <li><b>Вычисление полинома:</b> Метод Горнера с линейной сложностью <code>O(n)</code> многократно превосходит прямое наивное вычисление <code>O(n²)</code> за счет устранения повторных возведений в степень.</li>");
        sb.AppendLine("      <li><b>Возведение в степень:</b> Быстрые бинарные алгоритмы <code>QuickPow</code>, <code>RecPow</code> и <code>QuickPow1</code> (сложность <code>O(log n)</code>) кардинально эффективнее наивного последовательного умножения <code>O(n)</code>, требуя единицы микросекунд даже для больших степеней.</li>");
        sb.AppendLine("      <li><b>Факторы погрешности:</b> Небольшие колебания эмпирической кривой объясняются системными процессами ОС, работой планировщика потоков и управлением кэш-памятью процессора. Усреднение по 5 запускам нивелирует эти погрешности.</li>");
        sb.AppendLine("    </ol>");
        sb.AppendLine("  </div>");
        ...
        string combinedSortChartPath)
    {
        ...
        sb.AppendLine("Эмпирический анализ временной сложности алгоритмов, сопоставление теоретических оценок Big-O с экспериментальными машинными замерами и аппроксимация кривых методом наименьших квадратов.\n");

        sb.AppendLine("## 2. Сравнительный анализ алгоритмов сортировки\n");
        sb.AppendLine($"![Сравнение сортировок]({combinedSortChartPath})\n");

        sb.AppendLine("## 3. Результаты экспериментов по алгоритмам\n");

        foreach (var (algo, bench, chartPath) in results)
        {
            sb.AppendLine($"### {algo.Name} ({algo.TheoreticalComplexityLabel})\n");
            sb.AppendLine($"- **Группа:** {algo.Group}");
            sb.AppendLine($"- **Подобранный коэффициент c:** `{bench.FittedCoefficient:E3}`");
            sb.AppendLine($"- **Теоретическая зависимость:** `T(n) = c · f(n)`\n");
        ...
        sb.AppendLine("| Алгоритм | Класс сложности | Коэффициент c |");
        sb.AppendLine("|---|---|---|");
        foreach (var (algo, bench, _) in results)
        {
            sb.AppendLine($"| {algo.Name} | {algo.TheoreticalComplexityLabel} | `{bench.FittedCoefficient:E2}` |");
        }
        sb.AppendLine();

        sb.AppendLine("## 5. Выводы\n");
        sb.AppendLine("1. Экспериментальные данные подтверждают теоретические оценки асимптотической сложности.");
        sb.AppendLine("2. Наглядно продемонстрировано превосходство эффективных алгоритмов: Горнер O(n) над наивным полиномом O(n²), Quick/TimSort O(n log n) над BubbleSort O(n²), бинарного возведения в степень O(log n) над наивным O(n).");
